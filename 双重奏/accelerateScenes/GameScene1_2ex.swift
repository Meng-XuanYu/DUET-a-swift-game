...
import SpriteKit
import GameplayKit
class GameScene1_2ex: SKScene, SKPhysicsContactDelegate, UIGestureRecognizerDelegate
{
    //声明游戏场景中的各个属性和节点
    ...
    override public func update(_ currentTime: TimeInterval)
    {
        ...
        if (Int(timer) == 17)
        {
            levelCompleted(level: 1, isAccelerated: true)
            if let view1_3 = self.view {
                        // 创建主页面场景
                        if let game1_3 = GameScene1_3ex(fileNamed: "GameScene1_3ex") {
    ...
    func die(ball: SKSpriteNode)
    {
        ...
        //创建一个新的游戏场景
        let scene = GameScene1_2ex(fileNamed: "GameScene1_2ex")
