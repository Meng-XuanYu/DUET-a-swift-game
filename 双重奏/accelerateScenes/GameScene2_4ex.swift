...
import SpriteKit
import GameplayKit
class GameScene2_4ex: SKScene, SKPhysicsContactDelegate, UIGestureRecognizerDelegate
{
    //声明游戏场景中的各个属性和节点
    ...
    override func didMove(to view: SKView)
    {
        ...
        blocksNode.run(SKAction.repeatForever(SKAction.moveBy(x: 0.0, y: -830.0, duration: 1.5)))
    ...
    override public func update(_ currentTime: TimeInterval)
    {
        ...
        if (Int(timer) == 22)
        {
            levelCompleted(level: 6, isAccelerated: true)
            if let view3_1 = self.view {
                        // 创建主页面场景
                        if let game3_1 = GameScene3_1ex(fileNamed: "GameScene3_1ex") {
    ...
    func die(ball: SKSpriteNode)
    {
        ...
        //创建一个新的游戏场景
        let scene = GameScene2_4ex(fileNamed: "GameScene2_4ex")
