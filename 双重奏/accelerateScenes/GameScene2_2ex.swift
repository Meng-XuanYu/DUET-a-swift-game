...
import SpriteKit
import GameplayKit
class GameScene2_2ex: SKScene, SKPhysicsContactDelegate, UIGestureRecognizerDelegate
{
    //声明游戏场景中的各个属性和节点
    ...
    override func didMove(to view: SKView)
    {
        ...
        blocksNode.run(SKAction.repeatForever(SKAction.moveBy(x: 0.0, y: -900.0, duration: 1.5)))
    ...
    override public func update(_ currentTime: TimeInterval)
    {
        ...
        if (Int(timer) == 15)
        {
            levelCompleted(level: 4, isAccelerated: true)
            if let view2_3 = self.view {
                        // 创建主页面场景
                        if let game2_3 = GameScene2_3ex(fileNamed: "GameScene2_3ex") {
    ...
    func die(ball: SKSpriteNode)
    {
        ...
        //创建一个新的游戏场景
        let scene = GameScene2_2ex(fileNamed: "GameScene2_2ex")
