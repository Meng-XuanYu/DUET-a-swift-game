...
import SpriteKit
import GameplayKit
class GameScene2_1ex: SKScene, SKPhysicsContactDelegate, UIGestureRecognizerDelegate
{
    //声明游戏场景中的各个属性和节点
    ...
    override func didMove(to view: SKView)
    {
        ...
        blocksNode.run(SKAction.repeatForever(SKAction.moveBy(x: 0.0, y: -910.0, duration: 1.5)))
    ...
    override public func update(_ currentTime: TimeInterval)
    {
        ...
        if (Int(timer) == 11)
        {
            levelCompleted(level: 3, isAccelerated: true)
            if let view2_2 = self.view {
                        // 创建主页面场景
                        if let game2_2 = GameScene2_2ex(fileNamed: "GameScene2_2ex") {
    ...
    func die(ball: SKSpriteNode)
    {
        ...
        //创建一个新的游戏场景
        let scene = GameScene2_1ex(fileNamed: "GameScene2_1ex")
