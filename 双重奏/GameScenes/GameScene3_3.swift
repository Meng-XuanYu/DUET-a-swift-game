...
import SpriteKit
import GameplayKit
class GameScene3_3: SKScene, SKPhysicsContactDelegate, UIGestureRecognizerDelegate
{
    //声明游戏场景中的各个属性和节点 
    ...
    override public func update(_ currentTime: TimeInterval)
    {
        if (isMovable)
        {
            ...
            if (isTouching && !isFast)
            {
                ...
                {
                    rotateNode(node: rotatePoint, clockwise: false, speed: 0.05)
                }
                else if (rotationDirection == 1)
                {
                    rotateNode(node: rotatePoint, clockwise: true, speed: 0.05)
                }
            }
            //加速部分
            if (isTouching && isFast)
        ...
        if (Int(timer) == 57)
        {
            ...
            if let savedLevel = UserDefaults.standard.value(forKey: "currentLevel") as? Int {
                if savedLevel == 11 {
                    levelCompleted(level: 9, isAccelerated: true)
                } else {
                    UserDefaults.standard.set(11, forKey: "currentLevel")
                    if let end = self.view {
                                // 创建主页面场景
                                if let endScene = GameScene1_1ex(fileNamed: "GameScene1_1ex") {
                                    endScene.scaleMode = .aspectFill

                                    // 显示主页面场景
                                    end.presentScene(endScene)
                                }
                                end.ignoresSiblingOrder = true
                    }
                }
            }
        }
    ...
    func die(ball: SKSpriteNode)
    {
        ...
        //创建一个新的游戏场景
        let scene = GameScene3_3(fileNamed: "GameScene3_3")
