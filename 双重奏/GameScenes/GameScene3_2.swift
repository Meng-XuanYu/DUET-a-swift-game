...
import SpriteKit
import GameplayKit
class GameScene3_2: SKScene, SKPhysicsContactDelegate, UIGestureRecognizerDelegate
{
    //声明游戏场景中的各个属性和节点
    ...
    override public func update(_ currentTime: TimeInterval)
    {
        ...
        if (Int(timer) == 42)
        {
            if let savedLevel = UserDefaults.standard.value(forKey: "currentLevel") as? Int {
                if savedLevel == 9 {
                    UserDefaults.standard.set(10, forKey: "currentLevel")
                }
            } else {
                UserDefaults.standard.set(10, forKey: "currentLevel")
            }
    ...
    func die(ball: SKSpriteNode)
    {
        ...
        //创建一个新的游戏场景
        let scene = GameScene3_2(fileNamed: "GameScene3_2")
