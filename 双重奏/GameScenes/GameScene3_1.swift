...
import SpriteKit
import GameplayKit
class GameScene3_1: SKScene, SKPhysicsContactDelegate, UIGestureRecognizerDelegate
{
    //声明游戏场景中的各个属性和节点
    ...
    override public func update(_ currentTime: TimeInterval)
    {
        ...
        if (Int(timer) == 30)
        {
            if let savedLevel = UserDefaults.standard.value(forKey: "currentLevel") as? Int {
                if savedLevel == 8 {
                    UserDefaults.standard.set(9, forKey: "currentLevel")
                }
            } else {
                UserDefaults.standard.set(9, forKey: "currentLevel")
            }
    ...
    func die(ball: SKSpriteNode)
    {
        ...
        //创建一个新的游戏场景
        let scene = GameScene3_1(fileNamed: "GameScene3_1")
