...
import SpriteKit
import GameplayKit
class GameScene2_4: SKScene, SKPhysicsContactDelegate, UIGestureRecognizerDelegate
{
    //声明游戏场景中的各个属性和节点
    ...
    override public func update(_ currentTime: TimeInterval)
    {
        if (isMovable)
        {
            ...
            if (isTouching && !isFast)
            {
                ...
                {
                    rotateNode(node: rotatePoint, clockwise: false, speed: 0.058)
                }
                else if (rotationDirection == 1)
                {
                    rotateNode(node: rotatePoint, clockwise: true, speed: 0.058)
                }
            }
            //加速部分
            if (isTouching && isFast)
    ...
    func die(ball: SKSpriteNode)
    {
        ...
        //创建一个新的游戏场景
        let scene = GameScene2_4(fileNamed: "GameScene2_4")
